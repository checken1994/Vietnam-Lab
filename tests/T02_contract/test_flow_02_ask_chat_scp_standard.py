...
from scp.api.chat import CHAT_RATE_LIMIT_MESSAGES, CHAT_RATE_LIMIT_WINDOW_SECONDS, MAX_CHAT_MESSAGE_CHARS, ConversationManager
...
def _no_crawl_thread_in_tests(data_dir: str='data'):
    logger.info(
        '[TEST-ISOLATION] AttackCrawler thread suppressed in T02 session')


def _no_fast_learning_thread_in_tests(*args, **kwargs):
    logger.info(
        '[TEST-ISOLATION] FastLearning background thread suppressed in T02 session'
        )
...
def _disable_openrouter(monkeypatch):
    ...
    for key in list(os.environ):
        if key.startswith(('OPENROUTER_', 'GROQ_', 'CEREBRAS_',
            'SAMBANOVA_', 'GEMINI_', 'NVIDIA_', 'GITHUB_')) or key in {
...
class _LocalOpenAICompatHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        is_judge_prompt = any('PASS or FAIL' in str(m.get('content', '')) for
            m in messages)
        content = ('PASS' if is_judge_prompt else
            'Đáp án fixture cục bộ: 2 cộng 2 bằng 4.')
...
def _start_local_openai_compat_server() ->ThreadingHTTPServer:
    ...
    guard.proof_store.record(provider='openai_compat', model=model,
        prompt_price=0, completion_price=0, catalog_hash=
        't02-fixture-catalog', observed_at=now.isoformat(), expires_at=(now +
        timedelta(hours=2)).isoformat(), evidence_id=
        'price://t02-local-fixture')
...
class TestFlow02AskEndpoint:
    ...
    def test_ask_endpoint_requires_valid_schema(self, monkeypatch):
        ...
        headers = {'Authorization':
            f"Bearer {create_access_token({'sub': 't02'})}"}
    ...
    def test_ask_endpoint_malware_detection_in_image_voice(self):
        ...
        assert detector._check_text('A calm landscape photo with mountains'
            ) is None
...
class TestFlow02WebSocketChat:
    ...
    def test_ws_chat_session_id_only_for_resume_not_auth(self, monkeypatch):
        ...
        with TestClient(app) as client:
            with client.websocket_connect(
                f'/chat?token={token}&session_id=t02resume1') as ws:
                welcome = ws.receive_json()
                assert welcome['session_id'] == 't02resume1'

    def test_ws_chat_fail_closed_when_no_answer_source_available(self,
        monkeypatch):
        ...
        assert [p.PROVIDER_NAME for p in get_gateway()._provider_chain(
            'chat') if p.enabled] == []
        with TestClient(app) as client:
            with client.websocket_connect(f'/chat?token={token}') as ws:
                ws.receive_json()
    ...
    def test_ws_chat_rate_limit_exceeded_closes_1008(self, monkeypatch):
        ...
        _disable_openrouter(monkeypatch)
        monkeypatch.setattr('scp.llm_gateway.client._gateway', None)
        monkeypatch.setattr(chat_module, 'CHAT_RATE_LIMIT_WINDOW_SECONDS', 
            3600.0)
        with TestClient(app) as client:
            with client.websocket_connect(f'/chat?token={token}') as ws:
                ws.receive_json()
...
class TestFlow02AskDetectorDegraded:
    ...
    def test_ask_local_unreachable_image_url_marks_detector_degraded(self,
        ...
        monkeypatch.setenv('SCP_JWT_SECRET', T02_JWT_SECRET)
        monkeypatch.setenv('SCP_WEB_FALLBACK', '0')
        monkeypatch.setenv('SCP_KERNEL_DB_PATH', str(tmp_path /
            'ask_kernel.sqlite3'))
        monkeypatch.setenv('SCP_KERNEL_TRACE_PATH', str(tmp_path /
            'ask_kernel_trace.jsonl'))
        _disable_openrouter(monkeypatch)
        monkeypatch.setattr('scp.llm_gateway.client._gateway', None)
        from scp.security.jwt_guard import create_access_token
        headers = {'Authorization':
    ...
    def test_ask_detect_degradation_flags_absent_when_no_media(self,
        monkeypatch, tmp_path):
        ...
        with TestClient(app) as client:
            _wait_until_ready(client)
            response = client.post('/ask', json={'question': 'hi'}, headers
                =headers)
            assert response.status_code == 200
            data = response.json()
...
class TestFlow02SlmsUrlSafety:
    ...
    def test_build_holiday_url_blocks_bad_country_code(self):
        ...
        assert build_holiday_url(2026, 'VN'
            ) == 'https://date.nager.at/api/v3/PublicHolidays/2026/VN'
        dd_slash = '.' * 2 + '/'
        dd_encoded = '.' * 2 + '%2F'
        for bad in (dd_slash, dd_encoded, 'V', 'VNX', 'V/', '/V', 'script',
            '', None):
    ...
    def test_city_and_bible_urls_encode_input_before_fetch(self):
        ...
        from scp.runtime.experts.url_builders import build_bible_url, build_city_search_url
        dots = '.' * 2
        traversal = dots + '/' + dots + '/etc/' + 'passwd'
        encoded_traversal = ('name=' + dots + '%2F' + dots + '%2Fetc%2F' +
            'passwd')
        city_url = build_city_search_url(traversal)
        assert city_url.startswith(
        ...
        path_segment = bible_url.split('https://bible-api.com/')[1].split('?')[
            0]
...
class TestFlow02V105RoutesFailClosed:
    ...
    def test_v105_history_stats_internal_error_500_no_leak(self,
        monkeypatch, tmp_path):
    ...
    def test_v105_calibration_accuracy_internal_error_500_no_leak(self,
        monkeypatch, tmp_path):
        ...
        monkeypatch.setattr(calibration_routes, '_DATA_DIR', tmp_path,
            raising=False)
...
class TestFlow02AskChatCausalCoverage:
    ...
    def test_causal_media_clean_scan_continues(self):
        ...
        from scp.security.image_voice_detector import ImageJailbreakDetector
        assert ImageJailbreakDetector()._check_text('Beautiful sunset today'
            ) is None
