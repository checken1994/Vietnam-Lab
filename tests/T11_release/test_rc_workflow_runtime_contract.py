...
def _step_using(steps: list[dict], action_prefix: str) ->dict:
    matches = [step for step in steps if str(step.get('uses', '')).
        startswith(action_prefix)]
    assert len(matches
        ) == 1, f'expected exactly one {action_prefix} step, found {len(matches)}'
    return matches[0]
...
def _step_named(steps: list[dict], name: str) ->dict:
    ...
    assert len(matches
        ) == 1, f'expected exactly one {name!r} step, found {len(matches)}'
    return matches[0]


def test_rc_promotion_declares_push_trigger() ->None:
    assert _has_push(_load(RC_WORKFLOW)
        ), 'authoritative RC workflow must continue to run on push'


def test_push_release_gates_provision_declared_runtimes_unconditionally(
    ) ->None:
    ...
    python_verify = str(_step_named(platform_steps,
        'Verify Python 3.12 runtime')['run'])
    node_verify = str(_step_named(platform_steps, 'Verify Node 20 runtime')
        ['run'])
    ...
    for workflow_path in sorted(WORKFLOW_DIR.glob('*.yml')):
        ...
        for job_name, job in data.get('jobs', {}).items():
            for step in job.get('steps', []):
                ...
                if not (uses.startswith('actions/setup-python@') or uses.
                    startswith('actions/setup-node@')):
                    continue
                condition = str(step.get('if', ''))
                assert 'workflow_dispatch' not in condition or 'push' in condition, f'{workflow_path.name}:{job_name} gates {uses} with {condition!r}; push would be able to skip runtime provisioning'
...
def test_pre_rc_mutation_budget_matches_authoritative_release_gate() ->None:
    ...
    pre_rc_steps = _load(PRE_RC_WORKFLOW)['jobs']['pre-rc-verification'][
        'steps']
    rc_command = str(_step_named(rc_steps, 'Live mutation testing gate')['run']
        )
    pre_rc_command = str(_step_named(pre_rc_steps,
        'Bounded live mutation score gate')['run'])
    ...
    for workflow_path in (RC_WORKFLOW, PRE_RC_WORKFLOW):
        ...
        for job_name, job in content.get('jobs', {}).items():
            for step in job.get('steps', []):
                ...
                for match in re.finditer('(tests/[\\w/]+\\.py)', cmd):
                    rel_path = match.group(1)
                    if not (root / rel_path).is_file():
                        missing.append(
                            f'{workflow_path}:{job_name} -> {rel_path}')
                for match in re.finditer(
                    'python\\s+((?:tools|scripts)/[\\w/]+\\.py)', cmd):
                    rel_path = match.group(1)
                    if not (root / rel_path).is_file():
                        missing.append(
                            f'{workflow_path}:{job_name} -> {rel_path}')
