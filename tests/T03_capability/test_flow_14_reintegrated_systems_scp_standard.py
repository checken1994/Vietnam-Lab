...
class TestReintegratedSystems:
    ...
    MOUNTED_ROUTERS = [('calibration', 'calibration_routes',
        '/v105/calibration'), ('forecast', 'forecast_routes',
        '/v105/forecast'), ('history', 'history_routes', '/v105/history'),
        ('risk_intelligence', 'risk_routes', '/v105/risk'), ('world_state',
        'world_state_routes', '/v105/world')]
    IMPORTED_IN_ENDPOINTS = [('capabilities', 'capabilities.voice',
        'VoiceHandler', 'api_server_parts/_ask_impl.py'), ('self_model',
        'self_model.capability_map', 'CapabilityMap',
        'api/routes/v106_routes.py'), ('consolidator',
        'consolidator.consolidator', 'KnowledgeConsolidator',
        'api/routes/v104_routes.py'), ('experience',
        'experience.semantic_kb', 'compute_tf_idf',
        'knowledge/domain_store.py'), ('policy', 'policy.retry_policy',
        'RetryPolicy', 'api_server_parts/lifespan.py'), ('release',
        'release.evidence_authority', 'ReleaseEvidenceAuthority',
        'api/routes/admin_v100.py')]
    STILL_ISOLATED = ['audit_engine', 'audit_r8', 'audit_r9', 'brain',
        'learning']
    ...
    @pytest.mark.parametrize('system_name,router_module,prefix',
        MOUNTED_ROUTERS)
    def test_mounted_router_exists(self, system_name, router_module, prefix,
        scp_root):
        """
        ...
        assert router_file.exists(
            ), f'Router module {router_module}.py missing for {system_name}'
        content = router_file.read_text(encoding='utf-8')
        assert 'APIRouter' in content, f'{router_module}.py does not define APIRouter'

    @pytest.mark.parametrize('system_name,router_module,prefix',
        MOUNTED_ROUTERS)
    def test_mounted_router_endpoint_responds(self, system_name,
        router_module, prefix, client):
        """
        [REINT-2] Mounted router endpoints respond (not 404).
        """
        test_endpoints = [f'{prefix}/health', f'{prefix}/status', f'{prefix}/']
        found = False
    ...
    def test_api_server_mounts_all_five_routers(self, scp_root):
        ...
        for system_name, router_module, _ in self.MOUNTED_ROUTERS:
            ...
            assert f'("{system_name}"' in content or f"'{system_name}'" in content, f'api_server.py does not mount {system_name} router'

    @pytest.mark.parametrize('system_name,import_path,class_name,file_path',
        IMPORTED_IN_ENDPOINTS)
    def test_imported_in_endpoint(self, system_name, import_path,
        class_name, file_path, scp_root):
        """
        ...
        assert f'from scp.{import_path}' in content or f'import scp.{import_path}' in content, f'{file_path} does not import {import_path}'
    ...
    def test_consolidator_v104_endpoint_exists(self, client):
        ...
        """
        resp = client.post('/v104/learn/consolidate', headers={
    ...
    def test_rag_v105_endpoint_exists(self, client):
        ...
        """
        resp = client.post('/v105/rag/query', json={'query': 'test'},
    ...
    def test_release_v100_endpoint_exists(self, client):
        ...
        """
        resp = client.get('/v100/release/evidence', headers={
            'Authorization': 'Bearer test'})
    ...
    def test_still_isolated_no_imports(self, dead_dir, scp_root):
        ...
        """
        import_pattern = re.compile(f'^(from|import)\\s+scp\\.{dead_dir}\\b')
            ...
            try:
                ...
                for line in content.splitlines():
                    ...
                    if import_pattern.search(stripped):
                        pytest.fail(
                            f'File {py_file.relative_to(scp_root.parent)} imports from still-isolated zone scp.{dead_dir}: {stripped}'
                            )
            except Exception:
                pass
    ...
    def test_audit_engine_directory_exists_but_isolated(self, scp_root):
        ...
        assert audit_engine_path.exists(
            ), 'audit_engine directory should exist'
    ...
    def test_audit_rx_no_python_files(self, dead_dir, scp_root):
        ...
        assert len(py_files
            ) == 0, f'{dead_dir} should have no Python files (data only)'
    ...
    def test_foundation_has_active_files(self, scp_root):
        ...
        for py_file in py_files:
            ...
            if content.strip() and py_file.name != '__init__.py':
                if 'deprecat' not in content.lower(
                    ) and 'DEPRECATED' not in content:
...
class TestReintegratedSystemsCausalCoverage:
    ...
    @pytest.mark.parametrize('system_name,router_module,prefix',
        TestReintegratedSystems.MOUNTED_ROUTERS)
    def test_causal_mounted_router_exists(self, system_name, router_module,
        prefix):
        """Branch: system mounted as router → endpoint responds"""
        router_file = Path('scp') / 'api' / 'routes' / f'{router_module}.py'
        assert router_file.exists(
            ), f'Router file {router_file} does not exist'

    @pytest.mark.parametrize('system_name,router_module,prefix',
        TestReintegratedSystems.MOUNTED_ROUTERS)
    def test_causal_mounted_router_responds(self, system_name,
    ...
    @pytest.mark.parametrize('system_name,import_path,class_name,file_path',
        TestReintegratedSystems.IMPORTED_IN_ENDPOINTS)
    def test_causal_imported_in_endpoint(self, system_name, import_path,
    ...
    def test_causal_still_isolated_no_imports(self, dead_dir):
        ...
        scp_root = Path('scp')
        import_pattern = re.compile(f'^(from|import)\\s+scp\\.{dead_dir}\\b')
        key_files = [scp_root / 'api_server.py', scp_root / 'api' /
            'routes' / 'admin_v100.py', scp_root / 'api' / 'routes' /
            'v104_routes.py']
